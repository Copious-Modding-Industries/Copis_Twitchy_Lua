	...
	if (sender_username:lower() == ModSettingGet("Copis_Twitchy_Lua.poweruser"):lower() or "") or ModSettingGet("Copis_Twitchy_Lua.pandemonium") then
		...
		if message:match("LUA: ") then
			...
			for _, string in ipairs({
				...
				if message:lower():match(string:lower()) then
					cancel = true
				end

				if (not cancel) or ModSettingGet("Copis_Twitchy_Lua.chaos") then
					...
					GamePrintImportant(sender_username .. "'s script was invalid!", "Used banned phrases.", "data/ui_gfx/decorations_biome_modifier/conductive.png")
				end
			end
		end
	end
